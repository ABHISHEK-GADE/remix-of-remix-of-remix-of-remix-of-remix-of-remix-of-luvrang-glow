  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden h-[240px] sm:h-[300px] md:h-[360px]">
        {featured?.image ? (
          <img src={featured.image} alt="Collections" className="w-full h-full object-cover" />
        ) : (
      ...
      <div className="container-luxury py-8 md:py-16">
        ...
        {isLoading ? (
          ...
          <p className="text-center font-body text-muted-foreground py-16">No collections available yet.</p>
        ) : (
          <div className="space-y-20">
            {/* Featured Collection */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 items-center animate-fade-up">
              <Link to={`/collections/${featured.handle}`} className="group relative overflow-hidden rounded-2xl aspect-[4/5]">
                {featured.image ? (
                  ...
                  <div className="w-full h-full bg-secondary flex items-center justify-center">
                    <span className="font-display text-3xl text-muted-foreground">{featured.title}</span>
                  </div>
                )}
              ...
              <div className="flex flex-col justify-center">
                ...
                <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground mb-3">{featured.title}</h2>
                ...
                <div className="flex items-center gap-3 mb-6">
                  <span className="font-body text-xs text-muted-foreground bg-secondary rounded-full px-3 py-1">{featured.count}+ designs</span>
                  <span className="font-body text-xs text-muted-foreground bg-secondary rounded-full px-3 py-1">Made to Order</span>
                </div>
                <Link to={`/collections/${featured.handle}`} className="inline-flex items-center gap-2 bg-primary text-primary-foreground font-body text-sm font-medium px-6 py-3 rounded-md hover:bg-primary/90 transition-colors w-fit">
                  ...
                  <div className="mt-8">
                    ...
                    <div className="flex gap-3 overflow-x-auto scrollbar-hide">
                      {featured.products.slice(0, 4).map((p: any) => (
                        <Link key={p.node.id} to={`/product/${p.node.handle}`} className="flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 rounded-lg overflow-hidden bg-secondary hover:ring-2 hover:ring-primary/30 transition-all">
            ...
            {rest.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
                  {rest.map((col, i) => (
                    <Link key={col.handle} to={`/collections/${col.handle}`} className="group relative overflow-hidden rounded-xl animate-fade-up" style={{ animationDelay: `${i * 0.08}s` }}>
                      <div className="aspect-[4/5] overflow-hidden">
                        {col.image ? (
                          ...
                          <div className="w-full h-full bg-secondary flex items-center justify-center">
                            <span className="font-display text-xl text-muted-foreground">{col.title}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5 md:p-6">
                        <span className="inline-block text-[9px] sm:text-[10px] uppercase tracking-widest font-body font-semibold text-accent mb-1.5">{col.count}+ designs</span>
            ...
            <section className="text-center py-12 px-6 rounded-2xl bg-primary/5 animate-fade-up">
              ...
              <p className="font-body text-muted-foreground mb-6 max-w-md mx-auto text-sm">Browse all our products across every collection in one place.</p>
              <Link to="/shop" className="inline-flex items-center gap-2 bg-primary text-primary-foreground font-body text-sm font-medium px-8 py-3.5 rounded-md hover:bg-primary/90 transition-colors">
